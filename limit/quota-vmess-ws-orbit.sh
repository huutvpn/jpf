...
cekfile1=$(cat /etc/cobek/cache/vmess-ws-orbit/$user | wc -l)
if [[ $cekfile1 -gt 0 ]]; then
echo > /dev/null
else
echo "0" > /etc/cobek/cache/vmess-ws-orbit/$user
fi
}    
    
run_sesi1() {
vbaru=$(echo -e "$[ $inb + $outb ]")        
vlama=$(cat /etc/cobek/cache/vmess-ws-orbit/$user)
...
echo "$vbaru" > /etc/cobek/cache/vmess-ws-orbit/$user
}
