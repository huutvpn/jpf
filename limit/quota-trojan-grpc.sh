...
run_file() {
cekfile=$(cat /etc/cobek/trojan/$user | wc -l)    
if [[ $cekfile -gt 0 ]]; then
echo > /dev/null
else
echo -e "0" > /etc/cobek/trojan/$user            
fi
cekfile1=$(cat /etc/cobek/cache/trojan-grpc/$user | wc -l)
if [[ $cekfile1 -gt 0 ]]; then
echo > /dev/null
else
echo -e "0" > /etc/cobek/cache/trojan-grpc/$user
fi
}    
    
run_sesi1() {
vbaru=$(echo -e "$[ $inb + $outb ]")        
vlama=$(cat /etc/cobek/cache/trojan-grpc/$user)
vtotal=$(cat /etc/cobek/trojan/$user)        
if [[ $vbaru -gt $vlama ]]; then
vvar=$(echo -e "$[ $vbaru - $vlama ]" | sed "s/-//g")
echo -e "$[ $vvar + $vtotal ]" > /etc/cobek/trojan/$user
else
echo -e "$[ $vbaru + $vtotal ]" > /etc/cobek/trojan/$user
fi
}
...
echo "$vbaru" > /etc/cobek/cache/trojan-grpc/$user
}
