...
cekfile1=$(cat /etc/cobek/cache/vless-ws/$user | wc -l)
if [[ $cekfile1 -gt 0 ]]; then
echo > /dev/null
else
echo "0" > /etc/cobek/cache/vless-ws/$user
fi
}    
    
run_sesi1() {
vbaru=$(echo -e "$[ $inb + $outb ]")        
vlama=$(cat /etc/cobek/cache/vless-ws/$user)
...
echo "$vbaru" > /etc/cobek/cache/vless-ws/$user
}
